 ...
 * <p>
 * <p>
 * A utility class to quickly implement DAO for all kind of objects that depend on a channel and implement Docable, e.g.
 * Channel,
 * Filter,
 * RankingDefinitions,
 * Concept
...
public class MongoDao<T extends Docable, I> {
    ...
    public List<T> readManyAsList(ObjectId channelId) {
        return readManyAsIterable(channelId).into(new ArrayList<>());
    }

    public MongoIterable<T> readManyAsIterable(ObjectId channelId) {
        Bson filter = Filters.eq("channelId", channelId);
        return readManyAsIterable(filter);
    }
